...
| **STM サマリー** | 通常チャットで短期メモリ（STM）が生成されているとき、メッセージ領域右上に**円形のサマリーボタン**が前面表示。クリックで現在セッションの圧縮サマリーをパネルで確認（外側クリックで閉じる）。 |

## Demo

<a href="https://www.youtube.com/watch?v=yXpoJL8H3h0">
  <img src="https://img.youtube.com/vi/yXpoJL8H3h0/hqdefault.jpg" alt="デモ動画（クリックで YouTube で再生）" width="100%">
</a>
...
- **Hub とチャンク** — Scratch を横断的な会話の主軸とし、話題転換でセグメント（チャンク）を自動分割。タイトル・要約・重要度・TTL 付きで圧縮表示し、チャンクレールからワンクリックジャンプ。Hub 専用の provisional STM と Recall あり。
...
- **STM サマリー表示（通常チャット）** — 会話が進むと LLM が短期メモリ（STM）を定期的に圧縮更新（間隔は設定で変更可能）。サマリーが存在すると、チャット欄右上（分岐パンくずより前面）に**差し色の円形ボタン**（白アイコン＋ドロップシャドウ）が表示され、クリックで現在セッションのサマリーをパネル表示。パネル外クリックで直ちに閉じる。Hub / Inbox では非表示。
...
| **STM summary** | In normal chat, when short-term memory (STM) exists, a **circular summary FAB** appears top-right over the message area; tap to read the compressed session summary in a panel (closes on outside click). |

## Demo

<a href="https://www.youtube.com/watch?v=yXpoJL8H3h0">
  <img src="https://img.youtube.com/vi/yXpoJL8H3h0/hqdefault.jpg" alt="Demo video (opens on YouTube)" width="100%">
</a>
...
- **Hub & chunks** — Scratch is the cross-session spine; topic shifts auto-segment into chunks with title, summary, importance, TTL, and a chunk rail for one-click jump. Hub-specific provisional STM and Recall.
...
- **STM summary view (normal chat)** — As the conversation progresses, the LLM periodically compresses short-term memory (STM) (interval configurable). When a summary exists, an **accent-filled circular button** (white icon, drop shadow) appears at the top-right of the chat, above branch breadcrumbs. One tap opens the current session summary in a panel; click outside to dismiss immediately. Hidden in Hub and Inbox.
